...
#runtime parameters
JAVA_MAP_NAME='HnnxsRemoteAuthFlds'
PKGNAME='hnnxs'
DEF_START_SURFIX_NUM=''
DEF_PREFIX='HnnxsRemoteAuthFldDef'
INPUT_PATH='/home/tom/tmp/hnnx_gen/p.txt'
LIMIT=55
OUTPUT_PATH='/home/tom/tmp/hnnx_gen/def/'
...
pattern = re.compile('.*\((......)\).*')
world = re.compile('^[a-zA-Z]+\s$')
inp = re.compile('.*输入\s$')
outp = re.compile('.*输出\s$')

def hasdefname(line):
    return pattern.match(line)

def checkdefine(line):
    return inp.match(line) or outp.match(line)

def checkfield(line):
    return world.match(line) and not ("(" in line)
